...
const Carousel = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement> & CarouselProps>(
  ({ orientation = "horizontal", opts, setApi, plugins, className, children, rtl = true, ...props }, ref) => {
        ...
        {Array.from({ length: slideCount }).map((_, index) => (
          <Button
            ...
            className={cn(
              ...
              selectedIndex === index ? "bg-primary" : "bg-muted-foreground/20 hover:bg-muted-foreground/40",
            )}
            onClick={() => api?.scrollTo(index)}
          >
            <span className="sr-only">{index === selectedIndex ? "Current slide" : `Go to slide ${index + 1}`}</span>
          </Button>
        ))}
      </div>
    )
